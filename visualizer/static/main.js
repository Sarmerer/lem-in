    ...
    $("#next").click(function () {
      if (ants_out < ants_total) {
        ...
        data.paths.forEach(function (path, p) {
          ...
          if (ants_on_paths[p] > 0 && get_edge_opacity(path.nodes[0], path.nodes[1]) < 1) {
...
function color_path(nodes, i) {
  ...
  for (j = 0; j < nodes.length; j++) {
    if (nodes[j] != cy.nodes('node[type = "start"]').id() && nodes[j] != cy.nodes('node[type = "end"]').id()) {
      room = nodes[j];
      cy.nodes('node[id = "' + nodes[j] + '"]').style("background-color", color);
    }
    cy.edges()
      .filter(function (edge) {
        var e1 = edge.source().id() == nodes[j] && edge.target().id() == nodes[j + 1];
        var e2 = edge.source().id() == nodes[j + 1] && edge.target().id() == nodes[j];
        return e1 || e2;
      })
